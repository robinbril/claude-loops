---
name: keten-loop
description: Orchestratie-chain die een einddoel end-to-end schipt door de loop-collectie aan elkaar te rijgen: sweep (recon) -> plan -> bouw-loop -> polijst-loop -> fix-loop -> onafhankelijke eind-verificatie. Het token-zuinige supergoal-alternatief: een evaluator-pass per gate in plaats van de volledige evaluator+council-machinerie, sonnet-workers overal waar het kan, alles op Claude-modellen. Gebruik bij "/keten-loop", "ship dit end-to-end maar goedkoper dan supergoal", of een niet-triviale feature die je niet wilt babysitten zonder een volle supergoal-run te betalen.
...
- State en learnings leven in `keten-loop/state.md`; verse agents lezen disk, niet de conversatie.
